...
$valid_str_arr = [];
$valid_str_arr[] = '- [历史由来](eBook/01.1.md)';
foreach ($a as $k => $v) {

    if (strstr($v, '- 第') !== false && strstr($v, '[') !== false) {
        preg_match('/\(.*\)/', $v, $match);
        $t = $match[0];
        $t1 =str_replace('.0','',$t);
        $t2 = str_replace($t,'',$v);
        ...
        $t2 = str_replace('[','',$t2);
        $t2 = str_replace(']','',$t2);
        $valid_str_arr[] = '- ['.trim($t2).']'.$t1;
    } elseif (strstr($v, '- 第') === false && strstr($v, '[') !== false) {
